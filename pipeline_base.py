...
import pyarrow.parquet as pq  # Correct import
import pandas as pd
...
# Ejemplo: Mostrar el primer grafo
...
def plot_graph(G):
    ...
    # Opcional: Extraer una característica para mostrar en las aristas
    edge_weights = [data['mod_order'] for _, _, data in G.edges(data=True)]

    # Dibujar nodos
    nx.draw_networkx_nodes(G, pos, node_size=300, node_color='lightblue')

    # Dibujar aristas con pesos (por ejemplo, mod_order)
    nx.draw_networkx_edges(G, pos, width=1.0, edge_color='gray')

    # Opcional: Dibujar etiquetas de los nodos
    nx.draw_networkx_labels(G, pos, font_size=10, font_family='sans-serif')

    # Mostrar la visualización
    plt.axis('off')
    plt.title('Grafo de Lightpaths - Muestra')
    plt.show()

# Suponiendo que ya has ejecutado el código anterior y tienes la lista de grafos
G = graphs[0]  # Grafo de la primera muestra
